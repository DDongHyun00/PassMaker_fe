...
const AuthModal = ({ onClose }) => {
    ...
    const handleFindEmail = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post("http://localhost:8080/api/users/find-email", {
                name,
                phone,
            });
                ...
                <div className="flex mb-4 border-b">
                    <button
                        className={`flex-1 py-2 font-semibold ${
                            activeTab === "email" ? "border-b-2 border-blue-500 text-blue-600" : "text-gray-500"
                        }`}
                ...
                {activeTab === "email" ? (
                    <form onSubmit={handleFindEmail}>
                        ...
                        <input
                            ...
                            required
                        />
                        <button className="w-full bg-blue-600 text-white p-2 rounded mt-1">이메일 찾기</button>
                    </form>
                ) : (
                    <form onSubmit={handleResetPassword}>
                        ...
                        <input
                            ...
                            required
                        />
                        <button className="w-full bg-blue-600 text-white p-2 rounded mt-1">임시 비밀번호 전송</button>
                    </form>
                )}

                {/* 결과 메시지 */}
                {result && <p className="mt-3 text-center text-sm text-green-600">{result}</p>}
